...
RUN apt-get update \
    && apt-get install -y hadoop-client hbase libssl-dev libffi-dev python-dev python-pip spark-core spark-history-server spark-python pig oozie-client impala-shell openssh-server \
    && apt-get clean \
    && apt-get autoclean \
